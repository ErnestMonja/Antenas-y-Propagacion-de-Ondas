...
Se tiene que en un eje cartesiano tridimensional definido por los ejes $x$, $y$ y $z$ tal como el que muestra la \autoref{fig:fig_1}:
\begin{figure}[H]
    \centering
...
Si se coloca una antena dipolo de longitud $L$ en el centro de los ejes de referencia, se puede desarrollar y obtener las magnitudes de los campos eléctricos y magnéticos en coordenadas polares según: [2]
\begin{equation}
    \begin{cases}
...
Nótese que en ambas ecuaciones de \autoref{eq_1}, que existe un termino común remarcado en corchetes el cual se denomina \textit{Factor de Forma} definido entonces por:
\begin{equation}
...
Reemplazando a $\beta_{0} = \frac{2\pi}{\lambda}$, se obtiene una expresión del \textit{Factor de Forma} la cual depende de la longitud de onda:
\begin{equation}
    \left| F(\theta) \right| = \left| \frac{\cos\left( \frac{\pi L}{\lambda}\cos(\theta) \right) - \cos\left( \frac{\pi L}{\lambda} \right)}{\sin(\theta)}\right|
    \label{eq_3}
\end{equation}

Como se ha mencionado en la introducción, las expresiones del Factor de Forma $F(\theta)$ son la base para trazar y definir los diagramas de radiación de una antena, los cuales se proponen investigar en la sección siguiente.
